...
<div class="object-cover rounded-lg drop-shadow-2xl">
    <div class="p-2 px-10 py-4 text-right bg-blue-900 rounded-t-lg md:px-10">
        <h1 class="pt-3 text-2xl font-bold text-center text-white">REMOVE COMPUTER ASSET</h1>
    </div>

    <div class="px-6 pb-6">
        <form action="{{ route('admin-remove', $getComputerAssets->id) }}" method="POST" autocomplete="off">
            @csrf
            <br>
            <span class="flex items-center justify-center text-lg font-semibold text-black">Are you sure you want to remove this Asset?</span>
            ...
            <button type="submit" name="submit" id="submit" class="w-full text-white bg-red-600 hover:bg-red-400 hover:duration-300 focus:outline-none font-medium rounded-lg text-md px-5 py-2.5 text-center duration-300">Remove Computer Asset</button>
        </form>
    </div>
</div>
